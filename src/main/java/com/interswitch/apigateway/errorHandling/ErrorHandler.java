...
//public ResponseEntity<ErrorResponse> decodingException(final Throwable ex, ServerWebExchange exchange) {
//        String path = exchange.getRequest().getPath().toString();
//        String eMessage = ex.getMessage();
//        String[] keys = StringUtils.substringsBetween(eMessage, "(", ")");
//        String key = (keys != null) ? "'" + keys[0] : "";
//        HttpStatus status = HttpStatus.BAD_REQUEST;
//        ErrorResponse response = new ErrorResponse(path, status.value(), WordUtils.capitalize(status.name().toLowerCase()), "Failed to read Http Message: cannot deserialize Enum instance " + key);
//        return handleExceptionInternal(response, status);
//        }
//
